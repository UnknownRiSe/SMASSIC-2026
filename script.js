...
// ============ LOAD MAPEL ============async function loadMapel() {
showL();
try {
...
if (violations > 0) { vb.className = 'vb warn'; vb.textContent = ' ' + violations + '/3'; }
...
<div class="ol">${o.l}</div><div class="ot">${o.t || ''}</div>
</div>`;
});
...
(u > 0 ? '<br><span style="color:var(--d)"> Masih ada <strong>' + u + '</strong> soal belum dijawab!</span>' : '') +
...
.from('HASIL')
.insert([insertData])
.select();console.log('[SUBMIT] Response:', { status, statusText, data, error });
if (error) {
...
window.addEventListener('blur', function () {if (!examActive || examSubmitted || isBlocked) return;
