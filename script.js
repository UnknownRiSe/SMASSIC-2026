// ============================================================
// CBT SMASSIC 2026 — SISWA CLIENT (UPGRADED v2)
// Features: Image in options, BSK table, proper zoom, HOTS,
//           HTML table in soal, keyboard nav, auto-save indicator
// ============================================================
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('inputNIS').addEventListener('keydown', e => {
  ...
  document.addEventListener('keydown', e => {
    ...
    if (isIsian) return;
    if (e.key === 'ArrowLeft' && currentIndex > 0) { prevSoal(); e.preventDefault(); }
    if (e.key === 'ArrowRight' && currentIndex < soalList.length - 1) { nextSoal(); e.preventDefault(); }
  });
});
...
async function doLogin() {
  ...
  try {
    const { data, error } = await sb.from('SISWA')
      ...
      .eq('NIS', nis).eq('Password', pw).maybeSingle();
    hideL();
...
function doLogout() {
  if (examActive) {
    toast('Selesaikan ujian terlebih dahulu!', 'warning');
    return;
  }
  if (currentSiswa) {
  ...
  try {
    ...
    if (data && (data.Status || '').toUpperCase() === 'BLOCKED') {
      document.getElementById('recheckInfo').textContent =
        'Masih diblokir. (' + new Date().toLocaleTimeString('id-ID') + ')';
    } else {
      fullResetState();
  ...
  try {
    ...
    if (data && (data.Status || '').toUpperCase() === 'KICKED') {
      document.getElementById('recheckKickInfo').textContent =
        'Masih dikeluarkan. (' + new Date().toLocaleTimeString('id-ID') + ')';
    } else {
      fullResetState();
...
async function startExam(mapel) {
  ...
  try {
    ...
    try {
      ...
      if (peng) {
        if (peng.Status_ujian === 'SELESAI') {
          hideL();
          toast('Ujian ' + mapel + ' sudah berakhir.', 'error');
          return;
        }
        if (peng.Status_ujian === 'BELUM') {
          hideL();
          toast('Ujian ' + mapel + ' belum dimulai.', 'warning');
          return;
        }
        shouldShuffle = peng.Acak_soal !== false;
        if (peng.Waktu_selesai) {
          const remaining = Math.max(0,
            Math.floor((new Date(peng.Waktu_selesai).getTime() - Date.now()) / 1000));
          if (remaining <= 0) {
            hideL();
            toast('Waktu ujian habis.', 'error');
            return;
          }
          durasi = remaining;
        } else {
    ...
    const { data: existing } = await sb.from('HASIL')
      .select('id').eq('NIS', currentSiswa.NIS).eq('Mapel', mapel).maybeSingle();
    if (existing) {
      hideL();
      toast('Anda sudah mengerjakan ujian ' + mapel, 'warning');
      return;
    }

    // UPGRADED: Added Is_HOTS to SELECT
    const { data: soalRaw, error: soalErr } = await sb.from('SOAL')
      .select('No, Soal, Opsi_A, Opsi_B, Opsi_C, Opsi_D, Opsi_E, Kunci, Bobot, Tipe, Gambar, Gambar_A, Gambar_B, Gambar_C, Gambar_D, Gambar_E, Sub_soal, Mapel, Is_HOTS')
...
function initExam(mapel, soal, durasiDetik) {
  ...
  document.getElementById('examStudentName').textContent =
    currentSiswa.Nama + ' (' + currentSiswa.NIS + ')';
  showPage('examPage');
  examActive = false;
...
function initZoomHandlers() {
  ...
  document.getElementById('zoomInBtn').addEventListener('click', () => {
    zoomScale = Math.min(5, zoomScale * 1.3);
    applyZoomTransform();
  });
  document.getElementById('zoomOutBtn').addEventListener('click', () => {
    zoomScale = Math.max(0.3, zoomScale / 1.3);
    applyZoomTransform();
  });
  ...
  document.addEventListener('mouseup', () => {
    ...
    if (c) c.style.cursor = zoomScale > 1 ? 'grab' : 'zoom-in';
  });
  ...
  // ESC to close zoom
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && isZoomingImage) {
      closeZoom();
    }
  });
}
...
function buildGrid() {
  ...
  grid.innerHTML = soalList.map((s, i) =>
    `<div class="soal-num" onclick="goToSoal(${i})">${i + 1}</div>`
  ).join('');
  updateGrid();
}
...
function updateGrid() {
  ...
  if (violations > 0) {
    vb.className = 'violation-badge warn';
    vb.textContent = '⚠ ' + violations + '/3';
  } else {
    vb.className = 'violation-badge clean';
    vb.textContent = '✅ Bersih';
  }
}

// ============ RENDER SOAL (PG, BS, IS, BSK) with HOTS + HTML table support ============
function renderSoal(index) {
  ...
  let hotsBadge = '';
  if (s.Is_HOTS) {
    hotsBadge = '<span class="tipe-badge" style="background:#fce7f3;color:#9d174d">⭐ HOTS</span>';
  }
  ...
  // Soal text - support HTML tables
  let soalTextHtml = '';
  const soalRaw = s.Soal || '';
  if (soalRaw.includes('<table') || soalRaw.includes('<TABLE')) {
    // Contains HTML table - render as HTML but sanitize dangerous tags
    soalTextHtml = sanitizeHTML(soalRaw);
  } else {
    soalTextHtml = esc(soalRaw);
  }
  ...
  if (tipe === 'PG') {
    ...
    optionsHtml = `<div class="options-list">${options.map(o => {
      ...
      if (o.img && o.img.trim()) {
        optImgHtml = `<div class="option-img-wrap">
          <img src="${esc(o.img.trim())}" alt="Opsi ${o.key}" class="option-img"
            data-zoomsrc="${esc(o.img.trim())}"
            onerror="this.style.display='none'">
        </div>`;
      }
      return `<div class="option-item${currentAnswer.toUpperCase() === o.key ? ' selected' : ''}"
        onclick="selectAnswer('${s.No}','${o.key}')">
    ...
    optionsHtml = `<div class="bs-options">
      <div class="bs-option${currentAnswer.toUpperCase() === 'BENAR' ? ' selected' : ''}"
        onclick="selectAnswer('${s.No}','Benar')">
        <div class="bs-icon">✅</div> Benar
      </div>
      <div class="bs-option${currentAnswer.toUpperCase() === 'SALAH' ? ' selected' : ''}"
        onclick="selectAnswer('${s.No}','Salah')">
        ...
        ${subs.map((sub, idx) => {
          ...
          return `<tr class="bsk-row">
            ...
            <td class="bsk-cell-radio">
              <div class="bsk-radio ${ans === 'BENAR' ? 'selected benar' : ''}"
                onclick="selectBSKAnswer('${s.No}','${sub.label}','Benar')">
                ${ans === 'BENAR' ? '✅' : '○'}
              </div>
            </td>
            <td class="bsk-cell-radio">
              <div class="bsk-radio ${ans === 'SALAH' ? 'selected salah' : ''}"
                onclick="selectBSKAnswer('${s.No}','${sub.label}','Salah')">
                ${ans === 'SALAH' ? '❌' : '○'}
              </div>
            </td>
  ...
  // Progress indicator
  const progress = Math.round(((index + 1) / soalList.length) * 100);
  ...
  card.innerHTML = `
    ...
    <div style="background:var(--border);border-radius:4px;height:4px;margin-bottom:16px;overflow:hidden">
      <div style="width:${progress}%;height:100%;background:var(--primary);border-radius:4px;transition:width 0.3s"></div>
    </div>
    <div class="question-text" id="qText">${soalTextHtml}</div>
    ${imgHtml}
    ${optionsHtml}
  `;
...
// Sanitize HTML - allow only safe tags (table, tr, td, th, thead, tbody, b, i, u, br, p, span, div, sup, sub, em, strong)
function sanitizeHTML(html) {
  const allowed = ['TABLE','TR','TD','TH','THEAD','TBODY','TFOOT','B','I','U','BR','P','SPAN','DIV','SUP','SUB','EM','STRONG','UL','OL','LI','CAPTION','COLGROUP','COL'];
  const allowedAttrs = ['style','class','colspan','rowspan','align','valign','width','border','cellpadding','cellspacing'];
  ...
  Array.from(temp.childNodes).forEach(child => clean(child));
...
function buildZoomableImg(src, alt) {
  return `<div class="question-img-wrap">
    <img src="${esc(src)}" alt="${esc(alt)}" class="question-img" data-zoomsrc="${esc(src)}"
      onerror="this.parentElement.innerHTML='<div style=\\'color:var(--danger);padding:12px\\'>⚠ Gambar gagal dimuat</div>'">
...
function startHeartbeat() {
  ...
  heartbeatInterval = setInterval(async () => {
    ...
    try {
      ...
      const { data } = await sb.from('SISWA')
        .select('Status').eq('NIS', currentSiswa.NIS).maybeSingle();
      if (data) {
...
function unsubscribeWarning() {
  if (warningChannel) {
    sb.removeChannel(warningChannel);
    warningChannel = null;
  }
}
...
function showWarningPopup(warning) {
  ...
  const titles = {
    YELLOW: 'Peringatan Kuning',
    ORANGE: 'Peringatan Oranye',
    RED: '🚨 PERINGATAN MERAH'
  };
  document.getElementById('warnIcon').textContent = icons[level] || '⚠';
  document.getElementById('warnTitle').textContent = titles[level] || 'PERINGATAN';
  document.getElementById('warnMsg').textContent =
    warning.Pesan || 'Anda mendapat peringatan dari pengawas.';
...
function setupAntiCheat() {
  document.addEventListener('copy', e => {
    ...
    e.preventDefault();
    reportCheat('Copy', 'Mencoba menyalin');
  });
  document.addEventListener('cut', e => {
    if (!examActive) return;
    e.preventDefault();
    reportCheat('Cut', 'Mencoba memotong');
  });
  document.addEventListener('paste', e => {
    ...
    if (!isIsian) {
      e.preventDefault();
      reportCheat('Paste', 'Mencoba menempel');
    }
  });
  document.addEventListener('contextmenu', e => {
    if (!examActive) return;
    e.preventDefault();
    reportCheat('Klik Kanan', 'Menu konteks');
  });
  document.addEventListener('keydown', e => {
    if (!examActive) return;
    const isIsian = e.target && (e.target.id === 'isianInput' || e.target.classList.contains('isian-input'));
    if (e.ctrlKey && !e.shiftKey && 'cvxauspi'.includes(e.key.toLowerCase())) {
      ...
      e.preventDefault();
      reportCheat('Shortcut Keyboard', 'Ctrl+' + e.key.toUpperCase());
      return false;
    }
    if (e.key === 'F12') {
      e.preventDefault();
      reportCheat('DevTools', 'F12');
      return false;
    }
    if (e.ctrlKey && e.shiftKey && 'ijc'.includes(e.key.toLowerCase())) {
      e.preventDefault();
      reportCheat('DevTools', 'Ctrl+Shift+' + e.key.toUpperCase());
      return false;
    }
    if (e.key === 'PrintScreen') {
      e.preventDefault();
      reportCheat('Screenshot', 'PrintScreen');
      blurContent();
      return false;
    }
    ...
    if (e.altKey && e.key === 'Tab') {
      e.preventDefault();
      reportCheat('Alt+Tab', 'Pindah aplikasi');
      return false;
    }
    if (e.key === 'Escape' && !isZoomingImage) {
      e.preventDefault();
  ...
  document.addEventListener('mouseleave', (e) => {
    ...
    if (e.clientY <= 5) {
      ...
      if (mouseLeaveCount >= 3) {
        reportCheat('Mouse Leave', 'Kursor meninggalkan area');
        mouseLeaveCount = 0;
      }
    }
  });
...
function blurContent() {
  ...
  if (eb) {
    eb.classList.add('content-blurred');
    setTimeout(() => eb.classList.remove('content-blurred'), 3000);
  }
}
...
async function reportCheat(jenis, detail) {
  ...
  if (violations >= 3) {
    ...
    document.getElementById('cheatMsg').textContent =
      detail + '. Pelanggaran ke-3! Akun diblokir.';
    document.getElementById('cheatCount').textContent =
      'Pelanggaran ke-' + violations + ' - DIBLOKIR';
...
function openSubmitModal() {
  ...
  soalList.forEach(s => {
    ...
    if (tipe === 'BSK') {
      const subs = s.Sub_soal || [];
      hasAns = subs.some(sub =>
        (jawaban[s.No + '_' + sub.label] || '').toString().trim() !== '');
    } else {
  ...
  document.getElementById('submitSummary').innerHTML =
    ...
    (unanswered > 0 ? '<br><span style="color:var(--danger)">⚠ ' + unanswered + ' soal belum dijawab!</span>' : '') +
  ...
  soalList.forEach(soal => {
    ...
    if (tipe === 'BSK') {
      ...
      subs.forEach(sub => {
        ...
        if (isHOTS) hotsTotalBobot += bobot;
        if (!ans) {
          kosong++;
        } else if (ans.toUpperCase() === kunci.toUpperCase()) {
          benar++;
          totalSkor += bobot;
          if (isHOTS) hotsBenar += bobot;
        } else {
          salah++;
        }
      });
...
function saveJawabanLocal() {
  ...
  try {
    localStorage.setItem('cbt_jawaban_' + currentSiswa.NIS + '_' + currentMapel,
      JSON.stringify({
        mapel: currentMapel,
        jawaban: jawaban,
        flagged: flagged,
...
function loadJawabanLocal() {
  ...
  try {
    ...
    if (Date.now() - d.time > 10800000) return null; // 3 hours expiry
    return d;
  } catch (e) { return null; }
}
...
function clearJawabanLocal() {
  if (!currentSiswa || !currentMapel) return;
  try {
    localStorage.removeItem('cbt_jawaban_' + currentSiswa.NIS + '_' + currentMapel);
  } catch (e) {}
}
...
window.addEventListener('beforeunload', e => {
  if (currentSiswa && !examSubmitted && examActive) {
    e.preventDefault();
    e.returnValue = 'Ujian berlangsung!';
